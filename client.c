...
void recv_msg_handler() {
  ...
  while (1) {
	...
	if(strcmp(temp_req, "GET_USER")==0){
		...
		json_object *from;
		json_object *delivered;
		json_object *at;
		
		json_object_object_get_ex(parsed_json,"body",&body);
		message_s = json_object_array_get_idx(body,0);
		from = json_object_array_get_idx(body,1);
		delivered = json_object_array_get_idx(body,2);

		printf("%s:%s at %s\n",json_object_get_string(from), json_object_get_string(message_s),json_object_get_string(delivered));
		str_overwrite_stdout();
