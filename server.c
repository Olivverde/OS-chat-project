				...
				if(strcmp(temp_req,"POST_CHAT")==0){
					...
					if(strcmp(aux_to,"all")==0){
						...
						for(i=0; i<MAX_CLIENTS; ++i){
							if(clients[i]){
								if(clients[i]->uid != uid){
									json_object *NEW_MESSAGE = json_object_new_object();
									json_object_object_add(NEW_MESSAGE, "response", json_object_new_string("NEW_MESSAGE"));
									json_object_array_add(param_arr,json_object_new_string(json_object_get_string(message)));
									json_object_array_add(param_arr,json_object_new_string(json_object_get_string(from)));
									json_object_array_add(param_arr,json_object_new_string(json_object_get_string(delivered_at)));
									json_object_array_add(param_arr,json_object_new_string(json_object_get_string(to)));
						...
						for(i=0; i<MAX_CLIENTS; ++i){
							if(clients[i]){
								//printf("%s:%s\n",clients[i]->name,name);
								if(strcmp(clients[i]->name,aux_to)==0){
									json_object *NEW_MESSAGE = json_object_new_object();
									json_object_object_add(NEW_MESSAGE, "response", json_object_new_string("NEW_MESSAGE"));
									json_object_array_add(param_arr,json_object_new_string(json_object_get_string(message)));
									json_object_array_add(param_arr,json_object_new_string(json_object_get_string(from)));
